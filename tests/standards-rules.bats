...
@test "TC-216: load.md references correct relative path to rules dir" {
    # load.md uses ../../ai/standards/rules/UNIVERSAL.md
    # From .claude/commands/ that resolves to ai/standards/rules/UNIVERSAL.md
    run grep -q "ai/standards/rules/UNIVERSAL.md" "$AI_SOURCE/templates/claude-code/commands/load.md"
    [ "$status" -eq 0 ]
}

@test "TC-217: load.md tech detection markers for all major languages" {
    local load_md="$AI_SOURCE/templates/claude-code/commands/load.md"

    # Each major language/infra must have a detection marker
    local markers="Cargo.toml pyproject.toml go.mod Dockerfile tsconfig.json ansible.cfg Chart.yaml"
    for marker in $markers; do
        grep -q "$marker" "$load_md" || {
            echo "FAIL: load.md missing tech detection marker: $marker"
            return 1
        }
    done
}
